...
!!! note
    Notation here may look confusing. One example to help understand is
...
in multi-class classification. This indicates that $h_\theta(x) = \mathbb{R}^K$. 

!!! note
    Usually, in our training sets {$(x^{(1)}, y^{(1)}), \dots, (x^{(m)}, y^{(m)})$}, we are given actual label (i.e. 
    $y^{(9)} = 10$ for handwritten digit recognition). However, we need to transform those labels into $\mathbb{R}^k$ by doing,for 
    instance, create $\mathbb{R}^{10}$ vector with last position being $1$ and rest being $0$
...
!!! note
    Notice that we don't compute $\delta_{(1)}$ because $\delta_{(1)}$ is associated with the input layer, which are
